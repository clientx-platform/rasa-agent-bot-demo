...
{  
  ...
  $create_message = send_to_clientx($account, $conversation, $bot_response->text);
}
...
function send_to_clientx($account, $conversation, $message){
  global $clientx_url, $clientx_bot_token; 
  $url = "{$clientx_url}/api/v1/accounts/{$account}/conversations/{$conversation}/messages";
  ...
  'http' => array(
      ...
      'header'=>  "Content-Type: application/json\r\n" .
                  "Accept: application/json\r\n" .
                  "api_access_token: {$clientx_bot_token}"
      )
  );

  $context  = stream_context_create( $options );
  $result = file_get_contents( $url, false, $context );
  error_log("clientx response: {$result}",0);
  $response = json_decode($result);
  return $result;
}
